...
namespace BGF.UnityHC
{
    public class HealthConnectManager : MonoBehaviour
    {
        ...
        [Tooltip("Call Init() automatically in Start(). Off by default — " +
                 "hook a UI Button to HealthConnectManager.Init() instead.")]
        public bool autoInitOnStart = false;

        [Tooltip("Call RequestPermissions() automatically once Init() succeeds. " +
                 "Off by default — hook a UI Button to RequestPermissions().")]
        public bool autoRequestPermissionsOnStart = false;
